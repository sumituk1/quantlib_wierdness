...
class Hedger:
    ...
    def __call__(self, msg):
        if isinstance(msg, Quote):
            ...
            hedges = self.hedge_calculator(msg, self.last_quotes, self.instrument_static)
...
def my_hedge_calculator(msg, lastquotes, instrument_static):
    ...
    if not hedge_listed_mapper == "":
        ...
        # contract_delta = dict()
        # contract_size = dict()
        # for sym in hedge_sym_arr:
        #     contract_size[sym] = instrument_static(sym=sym)['contract_size']
        #     contract_duration = instrument_static(sym=sym)['duration']
        #     contract_delta[sym] = FixedIncomeFuturesHedge.calculate_futures_delta(contract_size=contract_size,
        #                                                                           futures_duration=contract_duration)
