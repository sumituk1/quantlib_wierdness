...
class Euribor_1M:
    ...
    def createFraRate(self, fraRates):
        ...
        self.helpers += [ FraRateHelper(QuoteHandle(SimpleQuote(rate/100)),
                                        start, self.euribor1m)
                                        for rate, start in fraRates]

    # Finally, we add OIS quotes up to 30 years.
    def createSwaps(self, euribor1mSwapRates, discountCurve):
        ...
        self.helpers += [SwapRateHelper(QuoteHandle(SimpleQuote(rate/100)),
                                        ...
                                        self.euribor1m, QuoteHandle(), Period(0, Days),
                                        discount_curve)
                         for rate, tenor in euribor1mSwapRates]
    ...
    def bootStrapEuribor1MCurve(self, bootStrapMethod = BootStrapMethod.PiecewiseFlatForward):

        if bootStrapMethod == BootStrapMethod.PiecewiseLogCubicDiscount:
            self.euribor1m_c = PiecewiseLogCubicDiscount(0, TARGET(), self.helpers, Actual365Fixed())
            ...
            self.euribor1m_c.enableExtrapolation()
        elif bootStrapMethod == BootStrapMethod.PiecewiseFlatForward:
            self.euribor1m_c = PiecewiseFlatForward(0, TARGET(), self.helpers, Actual365Fixed())
            # Also, we enable extrapolation beyond the maturity of the last helper; that is mostly
            # for convenience as we retrieve rates to plot the curve near its far end.
            self.euribor1m_c.enableExtrapolation()
    ...
    @staticmethod
    def test_euribor_1m_bootstrap():
        # Create a EONIA discount curve
        eonia = eonia.OISCurve.test_eonia_bootstrap()
        ...
        euribor_1m.createFraRate(fraRate)
        ...
        euribor_1m.createSwaps(swapRates, eonia.eonia_c)

        euribor_1m.bootStrapEuribor1MCurve(BootStrapMethod.PiecewiseLogCubicDiscount)

        # Plot Euribor_6M curve curve
        spot = euribor_1m.euribor1m_c.referenceDate()
        dates = [spot + Period(i, Months) for i in range(0, 60 * 12 + 1)]
        rates = [euribor_1m.euribor1m_c.forwardRate(d, euribor_1m.euribor1m.maturityDate(d),
                                                    Actual360(), Simple).rate() for d in dates]
        return rates,dates

if __name__ == "__main__":
    rates,dates = Euribor_1M.test_euribor_1m_bootstrap()
    fig, ax = plt.subplots()
