...
class Euribor_3M:
    ...
    def test_euribor_3m_bootstrap():
        ...
        euribor_3m.createFraRate(fraRate)
        ...
        euribor_3m.createSwaps(swapRates, eonia.eonia_c)

        euribor_3m.bootStrapEuribor6MCurve(BootStrapMethod.PiecewiseLogCubicDiscount)

        # Plot Euribor_6M curve curve
        spot = euribor_3m.euribor6m_c.referenceDate()
        dates = [spot + Period(i, Months) for i in range(0, 60 * 12 + 1)]
        rates = [euribor_3m.euribor6m_c.forwardRate(d, euribor_3m.euribor6m.maturityDate(d),
                                                    Actual360(), Simple).rate() for d in dates]
        return rates,dates

if __name__ == "__main__":
    rates,dates = Euribor_3M.test_euribor_3m_bootstrap()
    fig, ax = plt.subplots()
