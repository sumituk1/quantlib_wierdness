    ...
    def test_case_6(self, plotFigure=False):
        ...
        for mk_msg in output_list:
            if mk_msg.trade_id == "222" and mk_msg.dt == '-900':
                ...
                self.assertLessEqual(np.abs((mk_msg.bps_markout - (-0.2439)) / mk_msg.bps_markout), tolerance, msg=None)
            elif mk_msg.trade_id == "222" and mk_msg.dt == '3600':
                self.assertLessEqual(np.abs(mk_msg.bps_markout - 0.4873) / mk_msg.bps_markout, tolerance, msg=None)