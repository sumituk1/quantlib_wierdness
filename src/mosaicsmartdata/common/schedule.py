# from src.main.python.risk.workalendar.usa import *
# from src.main.python.risk.workalendar.europe import *
...
class CSchedule:
    ...
    def get(self, input):
        ...
        if (self.validate_inputs(input) == True):
            ...
            while nextCouponDate < maturityDate:
                ...
                if (first):
                    coupon_rate = self.calculate_coupon_rate(coupon, prevCouponDate, datetime.now(), nextCouponDate,
                                                             frequency, day_count_conv)
                else:
                    coupon_rate = self.calculate_coupon_rate(coupon, prevCouponDate, None, nextCouponDate, frequency,
                                                             day_count_conv)
                schedules.append({
    ...
    def days_factor_2(self, nextCouponDate, maturity, day_count_conv, frequency):
        factor = 0.5
        if day_count_conv == DayCountConv.ACT_ACT:
            ...
            num = self.days_between_actual(nextCouponDate, maturity)
            dem = 360
            factor = num / dem

        return factor

    def days_factor(self, prevCouponDate, todayDate, nextCouponDate, maturity, day_count_conv, frequency):
        if day_count_conv == DayCountConv.ACT_ACT:
            if (prevCouponDate != None and nextCouponDate != None):
                ...
                num = self.days_between(todayDate, maturity)
                dem = self.days_between(todayDate, maturity) * self.get_frequency_factor(frequency)
            factor = num / dem
        elif day_count_conv == DayCountConv.ACT_360:
            if (prevCouponDate != None and nextCouponDate != None):
                num = self.days_between(prevCouponDate, nextCouponDate)
            ...
            if (prevCouponDate != None and nextCouponDate != None):
                ...
                num = self.days_between_actual(todayDate, maturity)
            dem = 360
            factor = num / dem
    ...
    def get_frequency_factor(self, freq):
        ...
        elif freq[-1:] == Frequency.WEEKLY:
            weeks = int(freq[:-2])
            freq_factor = 52 / weeks
        elif freq[-1:] == Frequency.SEMI:
            freq_factor = 2
        elif freq[-1:] == Frequency.ANNUAL:
            freq_factor = 1.0
        return freq_factor
