...
# Also - for calibrating NS curve: http://quant.stackexchange.com/questions/30604/how-to-construct-a-corporate-yield-curve
# NS curve fitting:- https://github.com/kirilldolmatov/QuantLib (http://quant.stackexchange.com/questions/28154/quantlib-fittedbonddiscountcurve-fitresults-error)
...
def calculateAccrued(dcf, coupon, freq, dayCount=ql.Thirty360, faceValue=100):
    ...
    return (coupon / Frequency.getFrequencyNumber(freq)) * faceValue * dcf # <-- dcf = (t2 - t1).days / (next_coupon - last_coupon).days
    # if freq == Frequency.ANNUAL:
